import type { PixMatch } from '../../stores/concOFXStore';
...
interface Props { matches: PixMatch[] }

export function PixTab({ matches }: Props) {
  if (matches.length === 0) {
    return (
      <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--muted)', fontSize: '0.8rem' }}>
        Nenhum lançamento PIX no período. Execute a conciliação primeiro.
      </div>
    );
  }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
          <thead>
            <tr style={{ background: 'var(--panel-alt)', borderBottom: '1px solid var(--border)' }}>
              {['Data', 'MEMO (OFX)', 'Valor OFX', 'NSU / Autorizção', 'Valor Sistema', 'Diferença', 'Status'].map(h => (
          ...
          <tbody>
            {matches.map(m => (
              ...
              >
                ...
                <td style={{ padding: '10px 14px', fontFamily: 'var(--font-mono)', color: 'var(--teal)', fontWeight: 600 }}>
                  {m.ofx ? fmtVal(m.ofx.amount) : '—'}
                </td>
                <td style={{ padding: '10px 14px', fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: 'var(--muted)' }}>
                  {m.sistema ? `${m.sistema.nsu} / ${m.sistema.autorizacao}` : '—'}
                </td>
                <td style={{ padding: '10px 14px', fontFamily: 'var(--font-mono)', color: 'var(--teal)', fontWeight: 600 }}>
                  {m.sistema ? fmtVal(m.sistema.valorLiquido) : '—'}
                </td>
